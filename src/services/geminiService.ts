...
- 숫자만 추출 ("%"(퍼센트), ","(콤마) 제거). 구분은 "경과조치전", "경과조치후"로 표기. 지급여력금액, 지급여력기준 금액은 테이블 우측 상단의 "단위" 를 참고하여 백만원(million)단위로 환산

2. 지급여력비율 중 기본/보완자본(K-ICS):
- 위치: "5-2. 지급여력비율" > "지급여력비율의 경과조치 적용에 관한 사항" > "공통적용 경과조치 관련" 테이블
- 대상: '경과조치 적용 전' 및 '경과조치 적용 후' 세트 1개만 추출
- 항목: 기본자본, 보완자본
- 숫자만 추출 ("%"(퍼센트), ","(콤마) 제거). 구분은 "경과조치전", "경과조치후"로 표기. 기본자본, 보완자본 금액은 테이블 우측 상단의 "단위" 를 참고하여 백만원(million)단위로 환산

3. 자본감소분 경과조치 (옵션):
- 위치: "5-2. 지급여력비율" > "지급여력비율의 경과조치 적용에 관한 사항" > "선택적용 경과조치 관련" 테이블
- 항목: 자본감소분 경과조치 적용금액 (경과조치전/경과조치후)
- 숫자만 추출 ("%"(퍼센트), ","(콤마) 제거). 구분은 "경과조치전", "경과조치후"로 표기. 자본감소분 경과조치 적용금액은 테이블 우측 상단의 "단위" 를 참고하여 백만원(million)단위로 환산
